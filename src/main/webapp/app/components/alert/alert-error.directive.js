(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('jhipsterhibernatetutorialApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-jhipsterhibernatetutorialApp-error');
                var entityKey = httpResponse.headers('X-jhipsterhibernatetutorialApp-params');
                if (errorHeader) {
